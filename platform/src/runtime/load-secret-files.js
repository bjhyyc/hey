...
const DEFAULT_SECRET_FILE_MAPPINGS = Object.freeze({
  ...
  // Production delivery-validation pins are not secrets, but they are computed
  // while the Worker image is built (client-tree/wrapper/runner checksums and
  // the frozen runner argument/environment JSON), so they use the same
  // file-indirection mechanism instead of being retyped by an operator.
  PETPACK_PRODUCTION_UPSTREAM_CLIENT_TREE_SHA256: "PETPACK_PRODUCTION_UPSTREAM_CLIENT_TREE_SHA256_FILE",
  PETPACK_PRODUCTION_ELECTRON_SHA256: "PETPACK_PRODUCTION_ELECTRON_SHA256_FILE",
  PETPACK_PRODUCTION_ELECTRON_RUNNER_SHA256: "PETPACK_PRODUCTION_ELECTRON_RUNNER_SHA256_FILE",
  PETPACK_PRODUCTION_ELECTRON_RUNNER_ARGS: "PETPACK_PRODUCTION_ELECTRON_RUNNER_ARGS_FILE",
  PETPACK_PRODUCTION_ELECTRON_CHILD_ENV: "PETPACK_PRODUCTION_ELECTRON_CHILD_ENV_FILE"
});
