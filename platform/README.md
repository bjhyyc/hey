...
This directory contains server-only, dependency-injected contracts for the product workflow. It deliberately contains no ModelArk key, Alipay key, administrator video prompt, public media URL, or customer-specific asset.
...
- `src/providers/alipay-payment-provider.js` — direct Alipay OpenAPI adapter; development simulation is not a second provider.
...
- `src/http/petpack-studio-http-api.js` — framework-neutral HTTP command adapter with server-injected identity, strict request shapes, safe response allowlists, and a raw Alipay form-notification boundary.
...
```js
{ acceptedCount: 2, allAcceptedNow: true, sourcePhotoRevisionId: "immutable-revision" }
```
...
proxy the raw Alipay callback. The checkout response includes only the newly
...
CloudBase, Alipay, ModelArk, or object storage. They cover authentication plus
