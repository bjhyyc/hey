...
- Redis 8 的 TLS-only、独立 CA、最小权限前缀 ACL、越界 key/`FLUSHALL` 拒绝已在隔离容器真实通过；`sent` outbox 对账与全新空队列命名空间的确定性重放也已通过，且未清空或删除 Redis 数据。本次 Alpine 镜像仍仅作功能验证；尚需选择并扫描正式不可变镜像、部署持久化实例，完成 AOF 保留重启、精确进程 hard-kill、死信与积压告警演练。
